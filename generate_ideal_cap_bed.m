%function [] = generate_ideal_cap_bed(num_hc_long, num_hc_high, Ncell_cap)
    ...
    if (mod(j,2) ~= 0)
        for i = 1:2:2*num_hc_high
            if (j == 1)
                ...
                next_origin = [floor(j/2)*(Lhc+a)+(a) i*b];
            end
        ...
        for i = 1:len
            if (j == 1)
                vess_seg = [vess_seg; 0 a origin(i,2) origin(i,2) Ncell_cap];
            ...
            if (j == num_hc_long)
                vess_seg = [vess_seg; origin(i,1)+a*hc_v2(1)+a*hc_v4(1)+a*hc_v6(1) origin(i,1)+a*hc_v2(1)+a*hc_v4(1)+a*hc_v6(1)+(a) origin(i,2)+a*hc_v2(2)+a*hc_v4(2)+a*hc_v6(2) origin(i,2)+a*hc_v2(2)+a*hc_v4(2)+a*hc_v6(2) Ncell_cap]; 
        ...
        for i = 1:num_hc_high-1
            next_origin = [(Lhc-b)+((j/2)-1)*(a+Lhc)+(a) 2*i*b];
            origin = [origin; next_origin];
        end
...
for i = 1:length(vess_seg)
    plot([vess_seg(i,1) vess_seg(i,2)], [vess_seg(i,3) vess_seg(i,4)], '.-', 'Color', [0.5 0.5 0.5], 'LineWidth', 6)
end
...
save('ideal_cap_bed_1x1.mat', 'vess_seg', '-v7')
