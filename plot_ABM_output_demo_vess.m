function [] = plot_ABM_output_demo_vess(file_name)
...
vidObj = VideoWriter(erase(file_name, '.mat'),'MPEG-4');
vidObj.FrameRate = 4;
vidObj.Quality = 100;
open(vidObj);
...
for t = 1:num_timesteps
    ...
    for v = 1:num_vess
        ...
        if (vess_diameter(v,t) ~= 0)
            plot([x1 x2], [y1 y2], 'Color', [1 0.55 0.55], 'LineWidth', vess_diameter(v, t)*10)
        end
    end
    ...
    for v = 1:num_vess
        ...
        plot([x1 x2], [y1 y2], 'k.:', 'MarkerSize', 15.0)
    ...
    for c = 1:length(curr_cells)
       ...
       zeta = curr_cells(c,4);
        ...
        global_pos = [x1; y1] + xi*L*Lhat;
        
        perp_vect = cross([Lhat; 0], [0; 0; 1]);
        perp_vect = perp_vect(1:2,:);
        
        theta = 2*(zeta - 0.5)*pi;
        perp_pos = sin(theta);
        
        global_pos = global_pos + perp_pos*perp_vect*vess_diameter(v,t)/10;
        
        
        plot(global_pos(1), global_pos(2), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r')
    end
    
    % Migation speed 10
    %title([num2str(time(t)) ' days '])
   
    % Migration speed 6
    %title([num2str(2*(t-1)) ' hours '])
